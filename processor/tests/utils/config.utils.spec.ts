...
describe('Test src/utils/config.utils.ts', () => {
  test('should return the correct configuration when all env vars are valid', () => {
    ...
    expect(config).toEqual({
      ...
      mollie: {
        ...
        enableCardComponent: process.env.ENBALE_MOLLIE_CARD_COMPONENT,
      },
    });
  });
  ...
  test('should throw an error when ENBALE_MOLLIE_CARD_COMPONENT is not defined', () => {
    delete process.env.ENBALE_MOLLIE_CARD_COMPONENT;
    expect(() => readConfiguration()).toThrow(CustomError);
  });
});
